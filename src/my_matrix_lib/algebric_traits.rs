...
pub trait Ring where Self : VectorSpace{
    ...
    fn r_powu<U:Integer + Unsigned + DivAssign + SubAssign + Copy>(&self,pow : U)->Self where Self : Sized + Copy{
        let mut pow = pow;
        let mut result = *self;
...
pub trait Field where Self : Ring{
...
impl<T> Ring for T where T : VectorSpace<Scalar = T>{
    fn r_mult(&self,rhs:&Self)->Self {
        self.scale(rhs)
    }
...
pub trait NthRootTrait where Self : Ring {
...
pub trait TrigFunc where Self : Ring {
    fn cos(self)->Self;
    fn sin(self)->Self;
    fn tan(self)->Self;
